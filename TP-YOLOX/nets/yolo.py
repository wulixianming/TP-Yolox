...
from nets.attention import CBAM, SE, ECA,CA_Block, EMA
...
class YOLOPAFPN(nn.Module):
    def __init__(self, depth=1.0, width=1.0, in_features=( "dark3", "dark4", "dark5"), in_channels=[256, 512, 1024],
        ...
        # '''注意力机制'''
        self.cbam1 = EMA(int(in_channels[1] * width))
        self.cbam2 = EMA(int(in_channels[0] * width))
        self.cbam3 = EMA(int(in_channels[0] * width))
        self.cbam4 = EMA(int(in_channels[1] * width))
        ...
        # self.reduce_conv2 = Conv(int(in_channels[1] * width), int(in_channels[0] * width), 1, 1, act=act)
        # self.C3_p2 = CSPLayer(
        #     int(2 * in_channels[0] * width),
        #     int(in_channels[0] * width),
        #     round(3 * depth),
        ...
        self.eca_1 = EMA(int(in_channels[2] * width))  # 对应dark5输出的1024维度通道
        self.eca_2 = EMA(int(in_channels[1] * width))  # 对应dark4输出的512维度通道
        self.eca_3 = EMA(int(in_channels[0] * width))  # 对应dark3输出的256维度通道


    def forward(self, input):
        out_features = self.backbone.forward(input)
        [feat1, feat2, feat3] = [out_features[f] for f in self.in_features]
        feat3 = self.eca_1(feat3)
        feat2 = self.eca_2(feat2)
        feat1 = self.eca_3(feat1)
