...
class IOUloss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # 尝试加入diou，ciou
            ...
            h_d = (pred[:, :2] - target[:, :2])[:, 1]
            d = w_d ** 2 + h_d ** 2
            sigma = torch.pow(w_d ** 2 + h_d ** 2, 0.5)
            sin_alpha = torch.abs(h_d) / sigma
            sin_beta = torch.abs(w_d) / sigma
            ...
            # Calculate the aspect ratio of the target bounding box
            aspect_ratio_gt = w_gt / h_gt
            theta = 4
            aspect_ratio_diff = torch.abs((w_pred / h_pred) - aspect_ratio_gt)
            alpha = 0.1
            shape_cost = torch.pow((1 - torch.exp(-1 * W_w)), theta) + torch.pow((1 - torch.exp(-1 * W_h)), theta) + alpha * aspect_ratio_diff
            size_factor = torch.exp(-torch.sqrt(w_gt * h_gt))
            siou = iou - (distance_cost + shape_cost) * 0.5
            siou = siou + size_factor
